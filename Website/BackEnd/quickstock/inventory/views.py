...
def daily_summary(request, date=None):
    ...
    quotations = SalesQuotation.objects.select_related("customer", "converted_invoice")
...
def super_admin_dashboard(request):
    ...
    Shows every subscribed company and key subscription health metrics.
    """
    if not request.user.is_superuser:
        messages.error(request, "Only the platform superuser can access this dashboard.")
        return redirect("dashboard")

    companies = (
        _company_admin_queryset()
        .annotate(
            staff_count=Count("staff_profiles", distinct=True),
            location_count=Count("user__locations", distinct=True),
            item_count=Count("user__inventory_items", distinct=True),
            customer_count=Count("user__customers", distinct=True),
            supplier_count=Count("user__suppliers", distinct=True),
            sale_count=Count("user__sales_owned", distinct=True),
            invoice_count=Count("user__sales_invoices_owned", distinct=True),
            quotation_count=Count("user__sales_quotations_owned", distinct=True),
            last_activity=Max("user__audit_logs__created_at"),
        )
        .order_by("-last_activity", "user__username")
    )
    ...
    trial_companies = companies.filter(plan="TRIAL").count()
    pro_companies = companies.filter(plan="PRO").count()
    free_companies = companies.filter(plan="FREE").count()
    expiring_soon_cutoff = timezone.now().date() + timedelta(days=7)
    expiring_soon_companies = companies.filter(
        status="active",
        plan="PRO",
        pro_expires__isnull=False,
        pro_expires__lte=expiring_soon_cutoff,
        pro_expires__gte=timezone.now().date(),
    ).count()

    company_rows = list(companies)
    total_staff = sum(company.staff_count or 0 for company in company_rows)
    total_locations = sum(company.location_count or 0 for company in company_rows)
    total_items = sum(company.item_count or 0 for company in company_rows)
    total_customers = sum(company.customer_count or 0 for company in company_rows)
    total_suppliers = sum(company.supplier_count or 0 for company in company_rows)
    total_sales = sum(company.sale_count or 0 for company in company_rows)
    total_invoices = sum(company.invoice_count or 0 for company in company_rows)
    total_quotations = sum(company.quotation_count or 0 for company in company_rows)
    attention_companies = [
        company for company in company_rows
        if company.status in {"pending", "suspended"}
        or (
            company.status == "active"
            and company.plan == "PRO"
            and company.pro_expires
            and timezone.now().date() <= company.pro_expires <= expiring_soon_cutoff
        )
    ][:6]
    recent_company_rows = company_rows[:5]
    attention_count = pending_companies + suspended_companies + expiring_soon_companies

    paginator = Paginator(company_rows, 20)
    company_page = paginator.get_page(request.GET.get("page"))
    ...
    return render(request, "inventory/super_admin_dashboard.html", context)
...
def customer_detail(request, pk):
    ...
    quotations = (
        SalesQuotation.objects.select_related("created_by", "converted_invoice")
...
def settings_view(request):
  ...
  # --- 4. Logic for Live Cash Register Tracking (ENHANCED) ---
    ...
    if is_admin:
        ...
        staff_profiles = UserProfile.objects.filter(parent_admin=profile).select_related("user", "default_location")
    ...
    context = {
        ...
        "all_staff": UserProfile.objects.filter(parent_admin=profile) if is_admin else [],
...
def _create_sales_invoice_from_quotation(user, owner_user, quote_id, selected_location):
    ...
    with transaction.atomic():
        ...
        if locked_quote.status != "draft":
            ...
            if existing:
                return existing, False, locked_quote
            raise ValueError("This quotation is no longer in draft state.")
        ...
        locked_quote.save(update_fields=["status", "converted_at"])
    ...
    if request.method == "POST":
        ...
        if customer_id:
            ...
            if not customer:
                ...
                messages.success(request, f"Customer '{customer.name}' created for this quotation.")
        ...
        if valid_until_raw:
            try:
                ...
                messages.error(request, "Invalid quote expiry date.")
        ...
        if not item_ids:
            messages.error(request, "Add at least one item to the quotation.")
            return redirect("sales_quotation_create")
        ...
        for idx, raw_item_id in enumerate(item_ids):
            ...
            item = item_map.get(item_id)
            if not item:
                messages.error(request, "One of the selected items is no longer available.")
                return redirect("sales_quotation_create")
        ...
        _log_action(
            ...
            {"quotation_id": quotation.id, "quote_no": quotation.quote_no, "total": str(total_amount)},
        )
...
def sales_quotation_convert_to_invoice(request, quote_id):
    ...
    _log_action(
        request.user,
        "invoice",
        "Sales quotation converted" if created_new else "Sales quotation conversion attempted (already converted)",
        {
    ...
    if created_new:
        messages.success(request, f"{quotation.quote_no} converted to invoice {invoice.invoice_no}.")
    else:
        messages.info(request, f"Quotation already converted. Opening {invoice.invoice_no}.")
    ...
    if request.method == "POST":
        ...
        if source_mode == "quotation":
            ...
            _log_action(
                request.user,
                "invoice",
                "Sales invoice created from quotation" if created_new else "Opened existing converted invoice",
                {
            ...
            if created_new:
                ...
                messages.info(request, f"{quotation.quote_no} is already converted. Opened {invoice.invoice_no}.")
